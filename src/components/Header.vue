<template>
  <div class="header">
    <van-button type="primary" @click="show = true" id="zzc" />
    <van-overlay :show="show" @click="show = false" class="van-overlay">
      <div class="wrapper" @click.stop>
        <h3>粥品香坊（回龙观）</h3>
        <van-rate
          class="star"
          v-model="value"
          :size="25"
          color="yellow"
          void-icon="star"
          void-color="#eee"
        />
        <div class="line">
          <div class="le"></div>
          <p>优惠信息</p>
          <div class="ri"></div>
        </div>
        <div class="block" />
      </div>
    </van-overlay>
    <div class="top">
      <div class="img">
        <img
          src="http://static.galileo.xiaojukeji.com/static/tms/seller_avatar_256px.jpg"
          alt=""
        />
      </div>
      <div class="content">
        <div class="t">
          <span class="brand"></span>
          <span class="title">粥品香坊（回龙观）</span>
        </div>
        <div class="m">蜂鸟专送/38分钟送达</div>
        <div class="b">
          <span class="icon"></span>
          <span>在线支付满28减5</span>
        </div>
      </div>
    </div>
    <div class="bot">
      <span class="font"></span>
      <span class="wz">
        粥品香坊其烹饪粥料的秘方源于中国千年古法，在融和现代制作工艺，由世界烹饪大师屈浩先生领衔研发。坚守纯天然、0添加的良心品质深得消费者青睐，发展至今成为粥类的引领品牌。是2008年奥运会和2013年园博会指定餐饮服务商。
      </span>
      <span class="iconfont icon-dayuhao dy"></span>
    </div>
  </div>
</template>

<script>
export default {
  data() {
    return {
      show: false,
      value: 1,
    };
  },
  methods: {
    // 方法
  },
  created() {
    // 实例被创建之后执行代码
  },
  mounted() {
    // 页面进入时加载内容
  },
  components: {
    // 组件的引用
  },
  props: {
    // 父辈向子辈传参
  },
  watch: {
    // 监测变化
  },
  computed: {
    // 计算属性
  },
};
</script>
<style lang='scss' scoped >
.header {
  height: 136px;
  background: url(http://static.galileo.xiaojukeji.com/static/tms/seller_avatar_256px.jpg)
    no-repeat;
  background-size: 100% 100%;
  #zzc {
    width: 100%;
    height: 136px;
    position: absolute;
    opacity: 0;
  }
  .van-overlay {
    background-color: rgba(7, 17, 27, 0.8) !important;
  }
  .wrapper {
    color: white;
    h3 {
      line-height: 16px;
      text-align: center;
      font-size: 16px;
      font-weight: 700;
      margin-top: 80px;
    }
    .star {
      margin-top: 30px;
      margin-left: 110px;
    }
    .line {
      margin-top: 40px;
      margin-left: 40px;
      .le {
        float: left;
        width: 110px;
        height: 2px;
        background-color: rgba(255, 255, 255, 0.2);
      }
      p {
        float: left;
        margin-top: -8px;
      }
      .ri {
        width: 110px;
        height: 2px;
        background-color: rgba(255, 255, 255, 0.2);
        float: left;
      }
    }
  }
  .top {
    height: 108px;
    background: rgba(7, 17, 27, 0.5);
    color: #fff;
    overflow: hidden;

    .img {
      float: left;
      width: 64px;
      height: 64px;
      margin-top: 20px;
      margin-left: 16px;
      img {
        width: 100%;
        height: 100%;
      }
    }
    .content {
      width: 260px;
      margin-top: 20px;
      margin-left: 10px;
      float: left;
      .t {
        display: flex;
        .brand {
          display: inline-block;
          width: 30px;
          height: 18px;
          background-image: url(data:image/png;base64,);
          background-size: 30px 18px;
          background-repeat: no-repeat;
          margin-right: 5px;
        }
        .title {
          font-weight: bold;
        }
      }
      .m {
        margin-top: 5px;
        font-size: 12px;
      }
      .b {
        font-size: 12px;
        margin-top: 8px;
        display: flex;
        align-items: center;
        .icon {
          margin-right: 5px;
          display: inline-block;
          width: 12px;
          height: 12px;
          background-size: 12px 12px;
          background-repeat: no-repeat;
          background-image: url(data:image/png;base64,);
        }
      }
    }
  }
  .bot {
    color: #fff;
    height: 28px;
    background: rgba(7, 17, 27, 0.7);
    display: flex;
    align-items: center;
    line-height: 28px;
    .font {
      margin-left: 3px;
      display: inline-block;
      width: 22px;
      height: 12px;
      background-size: 22px 12px;
      background-image: url(data:image/png;base64,);
      background-repeat: no-repeat;
    }
    .wz {
      margin-left: 3px;
      white-space: nowrap;
      overflow: hidden;
      text-overflow: ellipsis;
      font-size: 10px;
      width: 320px;
    }
  }
}
</style>